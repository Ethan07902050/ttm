...
class talkNet(nn.Module):
    def __init__(self, outputPath, lr = 0.0001, lrDecay = 0.95, weighted=False):
    ...
    def train_network(self, loader, epoch):
        ...
        for num, (audioFeature, visualFeature, labels) in enumerate(loader, start=1):
            ...
            audioEmbed, visualEmbed, weight = self.model.forward_cross_attention(audioEmbed, visualEmbed)
            if self.weighted:
                outsAV= self.model.forward_audio_visual_backend(audioEmbed, visualEmbed, weight)  
                outsA = self.model.forward_audio_backend(audioEmbed, weight)
                outsV = self.model.forward_visual_backend(visualEmbed, weight)
            else:
            ...
            sys.stderr.flush()  
        sys.stdout.write("\n")
        print(f"Train ACC : {top1 / index}, Train loss : {loss / len(loader)}")
        return loss/num, lr

    def valid_network(self, loader):
        self.eval()
        predAcc = 0
        predNum = 0
        predLoss = 0
        for audioFeature, visualFeature, labels in tqdm.tqdm(loader):
            with torch.no_grad():           
                ...
                loss, _, _, acc = self.lossAV.forward(outsAV, labels)    
                predAcc += acc
                predLoss += loss.item()
                predNum += len(labels)
        print(f"Valid ACC : {predAcc / predNum}, Valid loss : {predLoss / len(loader)}")
        torch.save(self.model.state_dict(), os.path.join(self.outputPath, f'model_{self.epoch}.pth'))
        if predAcc > self.bestAcc:
            self.bestAcc = predAcc
            print(f"=== Save model with best acc : {predAcc / predNum} ===")
            torch.save(self.model.state_dict(), os.path.join(self.outputPath, f'model_best.pth'))

    def test_network(self, loader):
        fileToPred = {}
        self.eval()
        for audioFeature, visualFeature, ids in tqdm.tqdm(loader):
            with torch.no_grad():                
                audioEmbed  = self.model.forward_audio_frontend(audioFeature[0].cuda())
                visualEmbed = self.model.forward_visual_frontend(visualFeature[0].cuda())
                audioEmbed, visualEmbed, weight = self.model.forward_cross_attention(audioEmbed, visualEmbed)
                ...
                outsAV = self.model.forward_audio_visual_backend(audioEmbed, visualEmbed)  
                score = self.lossAV.forward(outsAV)
                pred = score[0].argmax(dim=-1).cpu().item()
                fileToPred[ids[0][0]] = pred
        return fileToPred
    # def test_network(self, loader):
    #     self.eval()
    #     predScores = []
